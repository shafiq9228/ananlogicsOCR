package com.analogics.ui.settings;
    ...
    private class ReadThread extends Thread {
        ...
        public void run() {
            while(keep.get()){
                ...
                if(value != -1) {
                    String endchar = convertStringToHex(value);
//                    String str = toASCII(value);
                    sb.append(endchar);
                    mHandler.obtainMessage(SYNC_READ, endchar).sendToTarget();
                }
            }
        }
    ...
    private String convertStringToHex(int str) {

        // byte[] chars = str.toCharArray();
        StringBuffer hex = new StringBuffer();
        // for (int i = 0; i < str.length; i++) {

        hex.append(FixLengthOf(Integer.toHexString(str), 2));
        // }
    ...
    private String FixLengthOf(String val, int num) {
        if (val.length() == 1) {
            val = "0" + val;
        }
        return val;
